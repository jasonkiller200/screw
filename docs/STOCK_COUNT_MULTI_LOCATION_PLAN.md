...
*   **User Story 4.3.1: 更新訂單顯示頁面以顯示儲位**
    ...
    *   **進度與備註 (2025-10-24)：** 完成。已修改 review.html 和 review.js 以顯示儲位，並優化後端查詢。
...
*   **User Story 4.3.2: 調整訂單處理邏輯以使用儲位**
    ...
    *   **驗收標準：**
        ...
        *   庫存操作的準確性得到驗證。
    *   **進度與備註 (2025-10-24)：** 暫緩。原先對「庫存分配」的實作（預留庫存）因對業務流程理解有誤而撤銷。此故事的正確實作應與工單（work-orders）相關，或由 Epic 4.4 的入庫流程取代。
...
*   **User Story 4.4.1: 建立「待入庫訂單」列表頁面**
    ...
    *   **驗收標準：**
        *   系統能正確識別並顯示待入庫狀態的訂單。
        *   頁面能清晰展示待入庫訂單的關鍵資訊。
    *   **進度與備註 (2025-10-24)：** 完成。已建立後端路由、前端模板及主頁連結。
...
*   **User Story 4.4.3: 實現批量儲位指定與快速入庫後端邏輯**
    ...
    *   **驗收標準：**
        ...
        *   訂單項目中的已入庫數量得到更新。
    *   **進度與備註 (2025-10-24)：** 完成。已實現單品項入庫的完整前後端邏輯 (API, Service, DB)。批量入庫功能可在此基礎上擴展。
...
*   **User Story 4.4.4: 前端批量入庫操作介面與邏輯**
    ...
    *   **驗收標準：**
        ...
        *   批量入庫操作能成功執行，並顯示結果。
    *   **進度與備註 (2025-10-24)：** 完成。已實現批量入庫的後端 API、前端彈出視窗及對應的 JavaScript 邏輯。
...
*   **User Story 4.5.1: 截止日期後整張訂單入庫**
    *   **描述：** 系統應支援在**週期截止日期之後**，以整張採購訂單為單位，依照實際核准的入庫項目，依序執行入庫操作。
    *   **任務：**
        *   定義採購訂單的「待入庫」狀態和相關流程。
        *   開發後端邏輯，確保**只有在週期截止日期之後**才能執行入庫操作。
        *   開發後端邏輯，支援整張訂單的入庫操作，並更新所有相關訂單項目的狀態和庫存。
        *   設計前端介面，允許使用者選擇整張訂單進行入庫。
    *   **驗收標準：**
        *   使用者能夠在**截止日期之後**，選擇一張已核准的採購訂單進行整單入庫。
        *   **在截止日期之前，系統應阻止入庫操作。**
        *   整單入庫操作能正確更新所有相關零件的庫存數量和訂單項目狀態。
        *   系統能清晰顯示訂單的入庫進度。
...
### **其他修正與優化 (Other Fixes and Optimizations)**

*   **錯誤修復 (2025-10-24)：**
    *   修正 `controllers/weekly_order_controller.py` 中的 `SyntaxError` 和 `NameError`。
    *   修正資料庫遷移時的 `IntegrityError` 和 `OperationalError`，透過修改遷移腳本並執行資料修復腳本 `fix_null_locations.py`。
    *   修正 `static/js/weekly_orders/register.js` 中的 `ReferenceError`。
*   **UI/UX 優化 (2025-10-24)：**
    *   統一 `review.html` 和 `cycle_detail.html` 頁面中「儲位」欄位的顯示格式為「倉庫名稱 - 儲位代碼」。
    *   調整 `review.html` 和 `cycle_detail.html` 頁面中的表格欄位，將「類型」欄位顯示為零件種類，並新增「緊急程度」欄位顯示優先級。
    *   調整 `register.html` 和 `batch_register.html` 頁面中「申請類型」的標籤為「緊急程度」。
    *   優化採購訂單匯出 Excel 檔案的欄位內容與順序，並改進欄寬自動調整邏輯。
    *   優化從零件比較報告訂購時，自動帶入「儲位」和「類型」至申請單的功能。
    *   優化單筆申請頁面（`register.html`）的自動填入邏輯，使其在頁面載入時即可根據預填資料自動填入「儲位」和「類型」。
    *   優化批量申請頁面（`batch_register.html`）的自動填入邏輯，確保在預填資料時能正確觸發儲位和類型的動態載入。