...
def setup_database():
    """Create and populate the database with sample data for MVC architecture."""
    db_name = 'hardware.db'
    ...
    # Create parts table with additional fields for web interface
    cursor.execute('''
    CREATE TABLE parts (
    ...
    cursor.executemany(
        'INSERT INTO parts (part_number, name, description, unit, quantity_per_box, storage_location) VALUES (?, ?, ?, ?, ?, ?)',
        parts_to_insert
    )
