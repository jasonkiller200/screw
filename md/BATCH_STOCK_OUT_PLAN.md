...
-   **介面佈局**:
    -   **頂部 - 基礎資料區**:
        ...
        -   `[ ]` 備註 (文字區)。
    -   **中部 - 零件清單區**:
        -   `[ ]` 選擇倉庫後，透過 AJAX 載入該倉庫的庫存品項。
        -   `[ ]` 表格顯示品項，包含：核取方塊、零件編號、零件名稱、可用庫存、出庫數量輸入框。
    -   **底部 - 操作區**:
        -   `[ ]` 「確認批量出庫」按鈕。
-   **狀態**: `[ ] 未完成`
...
-   **核心功能**:
    -   `[ ]` 監聽倉庫下拉選單的變更事件，觸發 AJAX 請求以獲取庫存清單。
    -   `[ ]` 將獲取到的庫存資料動態渲染到表格中。
    -   `[ ]` 提供客戶端驗證（例如，出庫數量不能超過可用庫存）。
    -   `[ ]` 監聽「確認批量出庫」按鈕的點擊事件。
    -   `[ ]` 收集基礎資料和所有被勾選的品項及其出庫數量，打包成 JSON。
    -   `[ ]` 使用 `fetch` API 將 JSON 資料 POST 到 `/api/inventory/batch-stock-out`。
    -   `[ ]` 處理 API 的回傳結果，並在頁面上顯示成功或失敗的提示訊息。
-   **狀態**: `[ ] 未完成`
...
-   **修改內容**:
    -   `[ ]` 將 `href` 從 `url_for('web.stock_out')` 改為 `url_for('web.batch_stock_out')`。
-   **狀態**: `[ ] 未完成`
