...
def _get_parts_comparison_report_data():
    ...
    try:
        ...
        for demand in all_demands:
            ...
            # Attempt to get part name from existing inventory if available, otherwise use description
        ...
        for part_number, details in work_order_dict.items():
            ...
            if part_number not in inventory_dict:
                ...
                missing_in_inventory.append({
                    ...
                    'name': list(details['names'])[0] if details['names'] else details['description'], # Use collected name or description
        ...
        # 4.2 零件倉有，但無儲位的零件 (New Tab 4 Logic)
            ...
            if part_number in inventory_dict:
                ...
                if not part_obj.location_associations:
                    ...
                    demand_with_no_location.append({
                        ...
                        'part_id': part_obj.id, # Add part_id for frontend links
        ...
        # 4.3 零件倉有的零件與工單需求對比 (For Tabs 2 & 3)
        ...
        for part_number, part in inventory_dict.items():
            ...
            inventory_with_demand.append({
                ...
                'unit': part.unit or '',
                'category': part.type or '', # Add part type
                'warehouse_location_id': part.location_associations[0].warehouse_location_id if part.location_associations else None, # Add first associated location
        ...
        summary = {
            ...
            'demand_with_no_location_count': len(demand_with_no_location), # New Summary Stat
        ...
        return {
            ...
            'demand_with_no_location': sorted(demand_with_no_location, key=lambda x: x['total_required'], reverse=True) # New Data List
...
def parts_comparison_data():
    ...
    data = _get_parts_comparison_report_data()
...
def export_parts_comparison():
    ...
    report_data = _get_parts_comparison_report_data()

    if not report_data['success']:
        flash(f"匯出失敗: {report_data['error']}", 'error')
    ...
    header_fill_missing = PatternFill(start_color='FFC000', end_color='FFC000', fill_type='solid') # Orange for missing
    header_fill_shortage = PatternFill(start_color='FF0000', end_color='FF0000', fill_type='solid') # Red for shortage
    header_fill_sufficient = PatternFill(start_color='00B050', end_color='00B050', fill_type='solid') # Green for sufficient
    header_fill_no_location = PatternFill(start_color='00B0F0', end_color='00B0F0', fill_type='solid') # Blue for no location
    ...
    def add_sheet_data(ws, title, headers, data, header_fill):
        ...
        for col_idx, cell in enumerate(ws[1]):
            ...
            ws.column_dimensions[get_column_letter(col_idx + 1)].width = 15 # Default width
    ...
    print(f"DEBUG: Missing in Inventory count: {len(missing_data_raw)}")
    if missing_data_raw:
        # Transform missing_data to match headers and convert order_ids to string
        ...
        for item in missing_data_raw:
            missing_data_transformed.append({
                ...
                'order_ids': ', '.join(item['order_ids']) # Convert list to string
            })

        ws1 = wb.active
        add_sheet_data(ws1, '未建立零件', 
                       ['零件編號', '零件名稱', '需求數量', '工單號碼'],
                       missing_data_transformed, header_fill_missing)
    else:
        ws1 = wb.active
        ws1.title = '未建立零件'
        ws1.append(['無數據'])
    ...
    if shortage_data:
        ...
        add_sheet_data(ws2, '庫存不足',
                       ['零件編號', '零件名稱', '需求數量', '庫存數量', '缺貨數量', '單位', '工單號碼'],
    ...
    if sufficient_data:
        ...
        add_sheet_data(ws3, '庫存充足',
                       ['零件編號', '零件名稱', '需求數量', '庫存數量', '可用數量', '單位', '工單號碼'],
                       [{'part_number': item['part_number'], 'name': item['name'], 'required_quantity': item['required_quantity'],
    ...
    if no_location_data:
        ...
        add_sheet_data(ws4, '待建立儲位',
                       ['零件編號', '零件名稱', '總需求數量', '工單號碼'],
    ...
    return send_file(output,
                     mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                     as_attachment=True,
                     download_name=f'零件差異分析報告_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx')
