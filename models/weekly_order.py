...
from utils.datetime_utils import get_taipei_time  # 統一時區工具

# 向後相容：其他模組仍可透過 from models.weekly_order import User 引入
...
class OrderRegistration(db.Model):
    ...
    __table_args__ = (
        db.Index('ix_order_reg_part_status', 'part_number', 'status'),
        db.Index('ix_order_reg_cycle_status', 'cycle_id', 'status'),
    )
